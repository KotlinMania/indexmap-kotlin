# Claude Code Project Instructions — indexmap-kotlin

## Project Overview

This is **indexmap-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`indexmap`](https://crates.io/crates/indexmap).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/indexmap/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/indexmap>
