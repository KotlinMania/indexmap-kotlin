...
kotlin {
    ...
    iosX64 {
        binaries.framework {
            baseName = "Indexmap"
            // Paired into generated iOS fat-framework tasks, which require
            // every input framework in the pair to have the same linkage.
            isStatic = true
            xcf.add(this)
        }
    }
    ...
    swiftExport {
        moduleName = "Indexmap"
        flattenPackage = "io.github.kotlinmania.indexmap"
    }

    android {
        namespace = "io.github.kotlinmania.indexmap"
        compileSdk = 34
        minSdk = 24
...
mavenPublishing {
    ...
    coordinates(group.toString(), "indexmap-kotlin", version.toString())

    pom {
        name.set("indexmap-kotlin")
        description.set("Kotlin Multiplatform port of indexmap-rs/indexmap - A hash table with consistent order and fast iteration")
        inceptionYear.set("2026")
        url.set("https://github.com/KotlinMania/indexmap-kotlin")
        ...
        scm {
            url.set("https://github.com/KotlinMania/indexmap-kotlin")
            connection.set("scm:git:git://github.com/KotlinMania/indexmap-kotlin.git")
            developerConnection.set("scm:git:ssh://github.com/KotlinMania/indexmap-kotlin.git")
        }
    }
}
...
val codeqlCompileJvm = tasks.register<JavaExec>("codeqlCompileJvm") {
    ...
    doFirst {
        ...
        if (sourceFiles.isEmpty()) {
            val sentinelFile = sentinelDir.get().asFile.resolve("io/github/kotlinmania/indexmap/codeql/_CodeqlEmptySource.kt")
            ...
            sentinelFile.writeText(
                ...
                package io.github.kotlinmania.indexmap.codeql
