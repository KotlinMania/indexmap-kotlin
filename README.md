...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`indexmap`](https://crates.io/crates/indexmap).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `indexmap` crate will be cloned into `tmp/` (gitignored) when porting begins.
...
```kotlin
dependencies {
    implementation("io.github.kotlinmania:indexmap-kotlin:0.1.0-SNAPSHOT")
}
```
...
Original work copyrighted by the upstream `indexmap` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
