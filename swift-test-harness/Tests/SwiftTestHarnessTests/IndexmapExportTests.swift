...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Indexmap.swiftmodule/`
...
//      `import Indexmap` above would fail at compile time.
//
//   2. The static archive `libIndexmap.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Indexmap" }` and
//      `flattenPackage = "io.github.kotlinmania.indexmap"` configuration in
...
//      via the `IndexmapLibrary` product.
//
...
@Suite("Indexmap Swift Export Tests")
struct IndexmapExportTests {
    @Test("Swift module loads and exports cleanly")
    func testSwiftModuleLoads() throws {
        #expect(Bool(true), "Indexmap swift module imported cleanly")
    }
}
